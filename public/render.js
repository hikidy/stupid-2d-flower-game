...
(function () {
    ...
    const colors = {
        ...
        fleshPillarPink: "#994444",
        fleshPillarTendril: "#aa7777"
    }
    ...
    function drawBumpyPolygon(ctx, x, y, sides, radius) {
        const bumpAmount = radius * 0.35;
        ...
        for (let i = 0; i < sides; i++) {
            ...
            const cx = mx + nx * bumpAmount;
            const cy = my + ny * bumpAmount;
    ...
    function drawBabyAnt(ctx, x, y, r, t) {
        ...
        withTransform(ctx, x, y, s, () => {
            ...
            ctx.strokeStyle = FB(colors.ants, "#000000", 0.15);
            ctx.fillStyle = F(colors.ants);
    ...
    function drawHelCentipede(ctx, x, y, r, t, id) {
        withTransform(ctx, x, y, r, () => {
            ctx.lineWidth = 0.195;
            ...
            ctx.strokeStyle = FB(colors.hellMobColor, "#000000", 0.125);
            ctx.beginPath();
            ctx.arc(0, 0, 1, 0, TAU);
            ctx.fill();
            ctx.stroke();
    ...
    function drawRockMob(ctx, x, y, r, rands, rarity) {
        withTransform(ctx, x, y, r, () => {
            ...
            const pointCount = 5 + Math.floor(R(1) * 1.2 * R(2)) + rarity;
            ...
            // Spots clipped to rock
            ctx.save();
            ctx.clip();

            const spotCount = 4 + rarity;

            ctx.fillStyle = FB(colors.darkGray, "#ffffff", 0.31);

            for (let i = 0; i < spotCount; i++) {
                const ang = R(300 + i) * Math.PI * 2;
                const dist = 0.15 + R(350 + i) * 0.55;
    ...
    function drawFleshPillar(ctx, x, y, r, rands, rarity, t) {
        withTransform(ctx, x, y, r, () => {
            ctx.lineWidth = 2.32 / r;

            ctx.fillStyle = F(colors.fleshPillarTendril);
            ctx.strokeStyle = FB(colors.fleshPillarTendril, "#000000", 0.15);
            ...
            const radius = 1;
            const spikeCount = 8 + rarity + Math.floor(R(0) * 3);
            const spikeScale = 0.18 + rarity * 0.022;
            ...
            for (let i = 0; i < spikeCount; i++) {
                ...
                const px = -Math.sin(angle), py = Math.cos(angle);
                const len = radius * (spikeScale * (0.9 + R(52 + i * 3) * 1.6));
                const halfBase = radius * (0.255 + rarity * 0.005);
                const halfVar = radius * (0.018 + rarity * 0.0015);
                const half = halfBase + (R(90 + i) - 0.5) * 2 * halfVar;
                const inset = radius * (0.12 + R(20 + i) * 0.14);
                const bend = (R(170 + i) - 0.5) * radius * (0.10 + rarity * 0.01);
                const wobblePhase = R(200 + i) * Math.PI * 2;
                const wobbleAmount = len * 0.25;

                const wobble =
                    Math.sin(t * 4 + wobblePhase) *
                    wobbleAmount;
            ...
            ctx.fillStyle = F(colors.fleshPillarPink);
            ctx.strokeStyle = FB(colors.fleshPillarPink, "#000000", 0.15);
            ctx.save();
            ctx.beginPath();
            ...
            ctx.fillStyle = FB(colors.fleshPillarPink, "#000000", 0.075);
            ...
            for (let i = 0; i < spotCount; i++) {
                ...
                const pulse =
                    1 +
                    Math.sin(t * 3 + R(450 + i) * Math.PI * 2) *
                    0.15;
    ...
    function drawHelWasp(ctx, x, y, r, t, rarity) {
        const s = r;
        const wiggle = Math.sin(t * 5) * 0.025;
        const wing = Math.sin(t * 12) * 0.077;
        const angle = 45 * (Math.PI / 180);
        const angleB = 160 * (Math.PI / 180);
        ...
        withTransform(ctx, x, y, s, () => {
            ...
            // Base styles
            const hornetFill = F(colors.hellMobColor);
            const stinger = F(colors.stingerBlack);

            const tendrilCount = 2 + rarity;
            const baseX = 0.25;
            const baseY = 0;

            ctx.save();
            ctx.globalAlpha = 1;
            ctx.strokeStyle = colors.starfish;
            ctx.lineWidth = 0.145;
            ctx.lineCap = "round";

            for (let i = 0; i < tendrilCount; i++) {
                const phase = i * 1.9;
                const len = 0.9 + i * 0.15;

                const spread = (i - (tendrilCount - 1) / 2) * 0.22;

                const startX = baseX;
                const startY = baseY + spread / 2.75;
                ...
                for (let j = 1; j <= 6; j++) {
                    ...
                    const wig =
                        Math.sin(t2 + phase + j * 0.8) * 0.12 +
                        Math.cos(t2 * 0.7 + phase) * 0.09;

                    x += 0.18 + i * 0.01; // slight divergence per tendril
                    y += wig * (j / 6) + (i - 2) * 0.005; // subtle fan-out bias
            ...
            ctx.beginPath();
            ctx.ellipse(0, 0, 1, 0.667, 0, angle, -angle);
            ctx.arc(1.765, 0, 1.15, -angleB, angleB, true);
            ctx.fill();
    ...
    function drawStinkbug(ctx, x, y, r, t) {
        const a = Math.sin(t * 7) * 0.05;
        const wiggle = Math.sin(t * 5) * 0.025;
        withTransform(ctx, x, y, r, () => {
            ctx.lineWidth = 0.15;
            ctx.strokeStyle = F(colors.darkGray);
            ctx.beginPath();
            ctx.moveTo(-0.97 + a, 0.57 + a);
            ctx.quadraticCurveTo(0, 0, -0.97 + a, -0.57 - a);
            ctx.stroke();

            ctx.beginPath();
            ctx.moveTo(-0.17 + a, 1.27 + a);
            ctx.quadraticCurveTo(0, 0, -0.17 + a, -1.27 - a);
            ctx.stroke();

            ctx.beginPath();
            ctx.moveTo(0.57 + a, 0.97);
            ctx.quadraticCurveTo(0, 0, 0.57 + a, -0.97);
            ctx.stroke();

            ctx.strokeStyle = FB(colors.peaGreen, "#000000", 0.15);
            ctx.fillStyle = F(colors.peaGreen);
            ctx.beginPath();
            ctx.ellipse(-0.05, 0, 0.78, 0.99, 0, 0, TAU);
            ctx.fill();
            ctx.stroke();

            ctx.save();
            ctx.translate(0.3, 0);
            const path = new Path2D("m 0.417 0.3902 c -0.1667 0.3641 -0.0674 0.6332 -0.2341 0.2691 c -0.1667 -0.3641 -0.1667 -0.9545 0 -1.3186 c 0.1667 -0.3641 0.0674 -0.095 0.2341 0.2691 c 0.1667 0.3641 0.1667 0.4162 0 0.7804 z");
    ...
    function drawSilverfish(ctx, x, y, r, t) {
        const a = Math.sin(t * 12) * 0.25;
        const b = Math.sin(t * -12) * 0.215;
        const wiggle = Math.sin(t * 5) * 0.025;
        withTransform(ctx, x, y, r, () => {
            const bodyPath = new Path2D("m 1.0303 0 c 0 0.4152 -0.2778 0.7518 -0.7291 0.7518 c -0.4513 0 -1.4339 -0.175 -1.4339 -0.7518 c 0 -0.5769 0.9826 -0.7518 1.4339 -0.7518 c 0.4513 0 0.7291 0.3366 0.7291 0.7518 z");
            ...
            ctx.beginPath();
            ctx.moveTo(0, 0);
            ctx.quadraticCurveTo(-.987, 0, -1.42, 0.6);
            ctx.quadraticCurveTo(-.887, 0, 0, 0.25);
            ctx.closePath();
            ctx.stroke();
            ...
            ctx.fillStyle = F(colors.silverfish);
            ctx.strokeStyle = FB(colors.silverfish, "#000000", 0.2);
            ctx.fill(bodyPath);
            ctx.stroke(bodyPath);
            ...
            ctx.beginPath();
            ctx.moveTo(0.4, 0.46);
            ctx.quadraticCurveTo(0.25, 0, 0.4, -0.46);
            ctx.stroke();

            ctx.beginPath();
            ctx.moveTo(-0.05, 0.46);
            ctx.quadraticCurveTo(-0.12, 0, -0.05, -0.46);
            ctx.stroke();

            ctx.beginPath();
            ctx.moveTo(-0.6, 0.26);
            ctx.quadraticCurveTo(-0.45, 0, -0.6, -0.26);
            ctx.stroke();
    ...
    function drawSponge(ctx, x, y, r, rands) {
        withTransform(ctx, x, y, r, () => {
            ctx.lineWidth = 0.075;
            ...
            const spongeColor = color[Math.floor(rands[0] * color.length)];

            ctx.strokeStyle = FB(spongeColor, "#000000", 0.15);
            ctx.fillStyle = F(spongeColor);

            ctx.lineWidth = 0.15;
            drawBumpyPolygon(ctx, 0, 0, 16, 1);
            ctx.fill();
            ctx.stroke();

            // pores
            ctx.fillStyle = FB(spongeColor, "#000000", 0.25);
            ctx.strokeStyle = FB(spongeColor, "#000000", 0.4);
            ctx.lineWidth = 0.03;

            for (let i = 1; i < 10; i++) {
                const angle = rands[i] * Math.PI * 2;
                const dist = Math.sqrt(rands[(i + 1) % 10]) * 0.65;
                const size = 0.04 + rands[(i + 2) % 10] * 0.05;
    ...
    function drawDemon(ctx, x, y, r, t) {
        const angle = 45 * (Math.PI / 180);
        const angleB = 160 * (Math.PI / 180);
        withTransform(ctx, x, y, r, () => {
            ctx.lineWidth = 0.175;
            ...
            const teethCount = 6;     // how many spikes
            const ringR = 0.73;        // radius of the teeth circle
            const toothLen = 0.5;     // spike length toward center
            const toothW = 0.135;      // spike base width

            const arc = TAU / 4.9799;          // how wide the "tooth fan" is (same as your old spread)
            const centerAng = Math.PI;     // LEFT side of the teeth circle
            const step = (teethCount <= 1) ? 0 : arc / (teethCount - 1);

            for (let i = 0; i < teethCount; i++) {
                const gog = Math.sin(t * 3 + i) * 0.075;
    ...
    const yellowHatImg = new Image();
    yellowHatImg.src = "hat.png";
    ...
    function drawPetalArt(ctx, typeId, rarity, x, y, radius, opts = {}) {
        ...
        switch (typeId) {
            ...
            case "pincer": {
                ...
                ctx.beginPath();
                ctx.moveTo(-0.1, 0);
                ctx.quadraticCurveTo(0, 0.65, 0.5, 0);
                ctx.quadraticCurveTo(0, 0.325, -0.1, 0);
                ctx.fill();
                ctx.stroke();
            ...
            case "rice": {
                ...
                ctx.beginPath();
                ctx.moveTo(0, 1);
                ctx.quadraticCurveTo(0.5, 0, 0, -1);
                ctx.stroke();

                ctx.lineWidth = .5;
                ctx.strokeStyle = F(colors.white);
                ctx.beginPath();
                ctx.moveTo(0, 1);
                ctx.quadraticCurveTo(0.5, 0, 0, -1);
                ctx.stroke();
            ...
            case "georgeEgg": {
                if (!yellowHatImg.complete || yellowHatImg.naturalWidth === 0) return;

                ctx.drawImage(yellowHatImg, -2.5, -2.5, 5, 5);
                break;
            }
            case "yinYang": {
                ctx.lineWidth = 0.15;
            ...
            case "sand": {
                ctx.lineWidth = 0.6;
                ctx.fillStyle = F(colors.sandGold);
                ctx.strokeStyle = FB(colors.sandGold, "#000000", 0.15);
                drawPolygonPath(ctx, 6, 1.5, TAU / 3 * 2, 1, 1);
                ctx.fill();
                ctx.stroke();
                break;
            }
    ...
    const Render = {
        drawMob(ctx, mob, x, y, r) {
            ...
            switch (mob.type) {
                ...
                case "demon": out = drawDemon(ctx, 0, 0, r, animTime); break;
                ...
                case "centipede_hel": out = drawHelCentipede(ctx, 0, 0, r, animTime, mob.chainPrevId); break;
                ...
                case "wasp_hel": out = drawHelWasp(ctx, 0, 0, r, animTime, mob.rarity); break;
                ...
                case "silverfish": out = drawSilverfish(ctx, 0, 0, r, animTime); break;
                case "fleshPillar": out = drawFleshPillar(ctx, 0, 0, r, mob.randoms, mob.rarity, animTime); break;
                case "sponge": out = drawSponge(ctx, 0, 0, r, mob.randoms); break;
